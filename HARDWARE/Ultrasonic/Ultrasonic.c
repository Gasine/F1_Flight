...
void PWM_IN_Config(void)
{
  ...
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;        
	...
	TIM1_ICInitStructure.TIM_Channel = TIM_Channel_2; //选择输入端 IC1 映射到 TI1 上
	TIM1_ICInitStructure.TIM_ICPolarity = TIM_ICPolarity_Rising; //上升沿捕获
	TIM1_ICInitStructure.TIM_ICSelection = TIM_ICSelection_DirectTI; //映射到 TI1 上
	TIM1_ICInitStructure.TIM_ICPrescaler = TIM_ICPSC_DIV1; //配置输入分频,不分频
	TIM1_ICInitStructure.TIM_ICFilter = 0x01;//IC1F=0000 配置输入滤波器 不滤波
	TIM_ICInit(TIM1, &TIM1_ICInitStructure); //初始化 TIM1 输入捕获参数
	
	TIM_ITConfig(TIM1,TIM_IT_CC2,ENABLE);//允许捕获中断
	TIM_Cmd(TIM1,ENABLE ); //使能定时器 5
}
